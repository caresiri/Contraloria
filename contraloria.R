...
library(dplyr)
#### Extraer Datos ####
SIACSICOP1519 <- read_dta("/Users/carlossiri/Dropbox/Contraloria/adjudicacionesycompras.dta") # Read Stata data into R
...
##### generate new corpus #####
corpus <- VCorpus(VectorSource(techdata$DESC_BIEN_SERVICIO))
toSpace <- content_transformer(function (x , pattern ) gsub(pattern, " ", x)) #Eliminar caracteres especiales
corpus <- tm_map(corpus, toSpace, "/")#Eliminar caracteres especiales
corpus <- tm_map(corpus, toSpace, "@")#Eliminar caracteres especiales
corpus <- tm_map(corpus, toSpace, "\\|")#Eliminar caracteres especiales
