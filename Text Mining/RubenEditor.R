...
SIACSICOP1519$corrNoTech <- 0
for(i in 1:nrow(corNO))
{
  SIACSICOP1519$corrNoTech[grepl(paste("(^(?=.*\\b",as.String(corNO[i,1]), "\\b)(?=.*\\b", as.String(corNO[i,2]), "\\b))", sep = "")
                , lower, perl = TRUE)] = 1
}
SIACSICOP1519$corrNoTech <- as.numeric(SIACSICOP1519$corrNoTech)

corSI <- filter(correlaciones, Eliminar == "No")
SIACSICOP1519$corrNoTech <- 0
for(i in 1:nrow(corSI))
{
  SIACSICOP1519$corrSiTech[grepl(paste("(^(?=.*\\b",as.String(corSI[i,1]), "\\b)(?=.*\\b", as.String(corSI[i,2]), "\\b))", sep = "")
                                 , lower, perl = TRUE)] = 1
}
SIACSICOP1519$corrSiTech <- as.numeric(SIACSICOP1519$corrSiTech)
...
#SIACSICOP1519$tech <- 
 # ifelse(
  #  SIACSICOP1519$corrNoTech > 0,
   # "No", SIACSICOP1519$tech)

#SIACSICOP1519$tech <- 
 # ifelse(
  #  SIACSICOP1519$corrSiTech > 0,
   # "Si", SIACSICOP1519$tech)
## Full stats
